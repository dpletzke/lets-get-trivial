...
async function copyAllStaticFiles(staticDir, outputDir) {
  if (staticDir && staticDir.length) {
    await Promise.all(staticDir.map(async function (dir) {
      ...
      var localStaticPath = _path.default.resolve(currentStaticDir); // @ts-ignore


      if (await !_fsExtra.default.exists(localStaticPath)) {
        _nodeLogger.logger.error(`Error: no such directory to load static files: ${localStaticPath}`);

        process.exit(-1);
      }

      _shelljs.default.cp('-r', `${localStaticPath}/!(index.html)`, _path.default.join(outputDir, staticEndpoint));
    }));
...
async function buildStaticStandalone(options) {
  ...
  _nodeLogger.logger.info(`=> Cleaning outputDir ${outputDir}`);

  await (0, _trash.default)(outputDir, {
    glob: false
  });
